...
RSpec.describe Raif::ModelResponse, type: :model do
  describe "validations" do
    it "validates presence of llm_model_key" do
      model_response = described_class.new(response_format: "text")
      expect(model_response).not_to be_valid
      expect(model_response.errors[:llm_model_key]).to include("can't be blank")
    end

    it "validates that the llm_model_key is a valid model key" do
      model_response = described_class.new(response_format: "text", llm_model_key: "invalid_model_key")
      expect(model_response).not_to be_valid
      expect(model_response.errors[:llm_model_key]).to include("is not included in the list")
    end
  ...
  describe "#parsed_response" do
    context "with text format" do
      ...
      it "returns the trimmed text" do
        expect(model_response.parsed_response).to eq("This is a text response.")
      end
    end
    ...
    context "with json format" do
      context "with valid JSON" do
        ...
        it "parses the JSON" do
          expect(model_response.parsed_response).to eq({ "key" => "value", "array" => [1, 2, 3] })
        end
      end
      ...
      context "with JSON wrapped in code blocks" do
        ...
        it "removes the code block markers and parses the JSON" do
          expect(model_response.parsed_response).to eq({ "key" => "value" })
        end
      end
    end
    ...
    context "with html format" do
      context "with valid HTML" do
        ...
        it "cleans and returns the HTML" do
          expect(model_response.parsed_response).to eq("<div>\n<p>Hello</p>\n<p>World</p>\n</div>")
        end
      end
      ...
      context "with HTML wrapped in code blocks" do
        ...
        it "removes the code block markers and returns the HTML" do
          expect(model_response.parsed_response).to eq("<div><p>Hello</p></div>")
        end
      end
      ...
      context "with HTML containing empty text nodes" do
        ...
        it "cleans empty text nodes" do
          ...
          expect(model_response.parsed_response).to include("<div>")
          expect(model_response.parsed_response).to include("<p>Hello</p>")
          expect(model_response.parsed_response).to include("<p>World</p>")
        end
      end
